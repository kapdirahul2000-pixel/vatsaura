...
const config = {
  ...
  dataFile: path.resolve(
    process.cwd(),
    String(process.env.ADMIN_SECURITY_DATA_FILE || "server/data/admin-security.json")
  ),
  ...
  userDataFile: path.resolve(
    process.cwd(),
    String(process.env.USER_AUTH_DATA_FILE || "server/data/user-auth.json")
  ),
  smtp: {
...
app.listen(config.port, () => {
  console.info(`Admin security server listening on http://localhost:${config.port}`);
});
